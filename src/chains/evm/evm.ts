...
export function registerEvmTools(server: McpServer) {
  // Get your EVM address from private key in .env
  server.tool(
    "getMyEvmAddress",
    "Get your EVM address from private key in .env",
    {},
    async () => {
      try {
        if (!process.env.ETH_PRIVATE_KEY || process.env.ETH_PRIVATE_KEY === 'your-eth-private-key') {
          throw new Error('ETH_PRIVATE_KEY not found or not set in environment variables. Please run the account generation script.');
        }

        const wallet = new Wallet(process.env.ETH_PRIVATE_KEY);
        const address = wallet.address;

        // Get balances for major networks
        const balances = [];
        for (const [networkName, config] of Object.entries(NETWORKS)) {
          try {
            ...
            balances.push(`${config.name}: ${formattedBalance} ${config.currencySymbol}`);
          } catch (err) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Your EVM Address: ${address}\n\nBalances:\n${balances.join('\n')}\n\nExplorer Links:\n${Object.entries(NETWORKS).map(([name, config]) => `${config.name}: ${config.explorer}/address/${address}`).join('\n')}`,
            },
          ],
        };
      } catch (err) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Failed to get EVM address: ${error.message}`,
            },
          ],
        };
      }
  ...
  console.error('✅ EVM tools registration completed successfully!');
}