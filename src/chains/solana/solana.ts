...
// Lazy initialization of Solana connection
...
async function getTokenAccounts(walletAddress: string) {
  try {
    ...
    const tokenAccounts = await getSolanaConnection().getParsedTokenAccountsByOwner(
      owner,
      {
...
// Helper function to decode private key (supports both base58 and base64)
function decodePrivateKey(privateKeyString: string): Uint8Array {
  try {
    // Try base58 first (legacy format)
    return bs58.decode(privateKeyString);
  } catch (e) {
    try {
      // Try base64 (new format from account generator)
...
export function registerSolanaTools(server: McpServer) {
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        const privateKeyBytes = decodePrivateKey(process.env.SOLANA_PRIVATE_KEY);
        const keypair = Keypair.fromSecretKey(privateKeyBytes);
        const publicKey = keypair.publicKey.toString();

        // Get SOL balance
        const balance = await getSolanaConnection().getBalance(keypair.publicKey);
  ...
  server.tool(
    ...
    async ({ address }) => {
      try {
        ...
        const balance = await getSolanaConnection().getBalance(publicKey);
        const solBalance = balance / LAMPORTS_PER_SOL;
  ...
  server.tool(
    ...
    async ({ address, encoding = 'base64' }) => {
      try {
        const publicKey = new PublicKey(address);
        const accountInfo = await getSolanaConnection().getAccountInfo(
          publicKey,
          'confirmed'
        );
...
Explorer URL: https://explorer.solana.com/tx/${signature}`,
  ...
  console.error('✅ Solana tools registration completed successfully!');
}